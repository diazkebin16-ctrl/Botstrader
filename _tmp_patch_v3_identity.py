...
def replace_once(path, old, new):
    ...
    if s.count(old)!=1:raise SystemExit(f"{path}: patch anchor count={s.count(old)}")
    p.write_text(s.replace(old,new,1),encoding="utf-8")

managed='''\"\"\"Generated Automation V3 PAPER-only research rule surface.
...
'''
Path("managed_strategy_rules.py").write_text(managed,encoding="utf-8")

p="automation_v3_candidate_mapping.py"
replace_once(p,
...
replace_once(p,'        \"candidate_definition_sha256\": definition_sha,\n        \"target_population_sha256\": target_sha,\n',
