#!/usr/bin/env python3
"""Reproduce the frozen EUR/USD data and optimize only its BUY lane."""
...
os.environ.setdefault("DB_PATH", "/tmp/eurusd_buy_target_optimization.db")

from eurusd_buy_target_optimizer import optimize_eurusd_buy
...
OUTPUT_ROOT = Path(os.getenv("EURUSD_BUY_OPTIMIZATION_ROOT", "/tmp/eurusd_buy_target_optimization"))
...
async def main() -> int:
    ...
    print("EURUSD_BUY_TARGET download_start", flush=True)
    ...
    print(f"EURUSD_BUY_TARGET data_identity {json.dumps(identity, separators=(',', ':'))}", flush=True)
    ...
    report = optimize_eurusd_buy(episodes, start=START, end=END)
    ...
    output = OUTPUT_ROOT / "EURUSD_BUY_TARGET_OPTIMIZATION.json"
    output.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"EURUSD_BUY_TARGET final {json.dumps(report, separators=(',', ':'))}", flush=True)
    print(f"EURUSD_BUY_TARGET report_sha256 {sha256(output)}", flush=True)
    return 0
